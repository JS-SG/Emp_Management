...
const AddCategory = () => {
    ...
    const handleSubmit=(e)=>{
        e.preventDefault()
        axios.post('http://localhost:3001/add_category',{category})
        .then(result=>{
            if(result.data.Status){
                nav('/admin_dashboard/category')
            }else {
            ...
            <div className='p-3 rounded w-25 border'>
                ...
                <form onSubmit={handleSubmit}>
                    <div className='mb-3'>
                        ...
                        onChange={(e)=>setCategory(e.target.value)} className='form-ctrl rounded-0'></input>
                        </div>
